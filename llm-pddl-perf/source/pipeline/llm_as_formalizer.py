...
Parser.add_argument("--index_start", default="47", help="index to start generating result from (inclusive)")
...
OPEN_SOURCED_MODELS = ["meta-llama/Meta-Llama-3.1-8B-Instruct", "google/gemma-2-9b-it", "meta-llama/Llama-3.1-70B-Instruct", "google/gemma-2-27b-it", "meta-llama/Llama-3.1-405B-Instruct", "meta-llama/Llama-3.3-70B-Instruct", "deepseek-ai/DeepSeek-R1-Distill-Qwen-32B", "deepseek-ai/DeepSeek-R1-Distill-Llama-8B"]
...
if MODEL in OPEN_SOURCED_MODELS:
    if "Llama-3.1-405B-Instruct" in MODEL:
        ENGINE = HuggingEngine(model_id = MODEL, prompt_pipeline=LLAMA3_PIPELINE, use_auth_token=True, max_new_tokens=10000, model_load_kwargs={"device_map": "auto", "quantization_config": quantization_config})
    elif "gemma" in MODEL:
        ENGINE = HuggingEngine(model_id = MODEL, prompt_pipeline=GEMMA_PIPELINE, use_auth_token=True, max_new_tokens=10000)
    ...
    elif MODEL in ["qwen2.5-coder-7b-instruct", "qwq-32b", "qwen3-8b", "deepseek-r1-distill-qwen-7b", "deepseek-r1-distill-qwen-1.5b", "deepseek-r1-distill-llama-70b", "deepseek-r1-distill-qwen-14b", "deepseek-r1-distill-llama-70b"]:
        QWEN_API_KEY = "YOUR_QWEN_API_KEY"
        client = OpenAI(api_key=QWEN_API_KEY, base_url="https://dashscope.aliyuncs.com/compatible-mode/v1")
    elif MODEL in ["deepseek/deepseek-r1-distill-llama-70b","deepseek/deepseek-r1-distill-qwen-14b","qwen/qwen3-8b", "qwen/qwq-32b", "meta-llama/llama-4-maverick-17b-128e-instruct",
...
def run_formalizer_gpt(domain, data, problem, model, force_json=False):
    ...
    message = prompt + "Return the domain file an problem file as a JSON object in the following format:\n{\n  \"domain file\": ...,\n  \"problem file\":...\n}"
    ...
    return_string = completion.choices[0].message.content.replace("\\n\\","")
    ...
    df_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/formalize/{problem}/{problem}_{model_name}_df.pddl'
    df_rewrite_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/error_rag/{problem}/{problem}_{model_name}_df.pddl'
    df_refine_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/refine/{problem}/{problem}_{model_name}_df.pddl'
    df_rag_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/rag_refine/{problem}/{problem}_{model_name}_df.pddl'
    pf_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/formalize/{problem}/{problem}_{model_name}_pf.pddl'
    pf_rewrite_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/error_rag/{problem}/{problem}_{model_name}_pf.pddl'
    pf_refine_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/refine/{problem}/{problem}_{model_name}_pf.pddl'
    pf_rag_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/rag_refine/{problem}/{problem}_{model_name}_pf.pddl'

    if not os.path.exists(os.path.dirname(df_path)):
        os.makedirs(os.path.dirname(df_path))
    if not os.path.exists(os.path.dirname(df_rewrite_path)):
        os.makedirs(os.path.dirname(df_rewrite_path))
    if not os.path.exists(os.path.dirname(df_rag_path)):
        os.makedirs(os.path.dirname(df_rag_path))
    if not os.path.exists(os.path.dirname(df_refine_path)):
        os.makedirs(os.path.dirname(df_refine_path))
    ...
    with open(df_rewrite_path, 'w') as df:
        df.write(domain_file)

    with open(pf_rewrite_path, 'w') as pf:
        pf.write(problem_file)

    with open(df_refine_path, 'w') as df:
        df.write(domain_file)

    with open(pf_refine_path, 'w') as pf:
        pf.write(problem_file)

    with open(df_rag_path, 'w') as df:
        df.write(domain_file)

    with open(pf_rag_path, 'w') as pf:
        pf.write(problem_file)
...
def run_formalizer_qwen(domain, data, problem, model, force_json=False):
    ...
    if model in ['o1-preview', 'deepseek-reasoner', "qwen2.5-coder-7b-instruct", "qwen3-8b", "deepseek-r1-distill-llama-8b", "llama-4-scout-17b-16e-instruct"]:
        start_index = return_string.find('{')
        end_index = return_string.rfind('}')
        json_string = return_string[start_index:end_index + 1]
        return_dict = json.loads(json_string, strict=False)
