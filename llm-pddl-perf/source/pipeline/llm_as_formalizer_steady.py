...
Parser.add_argument("--index_start", default="14", help="index to start generating result from (inclusive)")
Parser.add_argument("--index_end", default="15", help="index to end generating result from (exclusive)")
...
def run_formalizer_gpt(domain, data, problem, model, doc, force_json=False):
    ...
    df_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/steady/{problem}/{problem}_{model_name}_df.pddl'
    pf_path = f'../../output/llm-as-formalizer/{domain}/{data}/{model_name}/steady/{problem}/{problem}_{model_name}_pf.pddl'
...
def run_formalizer_qwen(domain, data, problem, model, doc, force_json=False):
    ...
    if model in ["qwq-32b",'o1-preview', 'deepseek-reasoner', "qwen2.5-coder-7b-instruct", "qwen3-8b",
                 "deepseek-r1-distill-llama-8b", "llama-4-scout-17b-16e-instruct"]:
...
def run_gpt_batch(domain, model, data, index_start, index_end):
    ...
    with open("domain_data.jsonl","r",encoding="utf-8") as f:
        for line in jsonlines.Reader(f):
            doc.append(line["document"])
            example.append(line["example"])
    knowledge = ""
    for i in range(len(doc)):
        knowledge += f"document:\n{doc[i]}\nexample:\n{example[i]}\n\n"
